...
monogon and self-bigon tables and, on request, triangles on three distinct untyped
support points. Thus its output is only an obstruction screen.
...
The displayed labels mu^k indicate the intended polygon order only; identifying
these finite predicates with the relevant A-infinity operations requires the
additional analytic, grading, branch-order, completeness, and convergence input
listed in the paper.
"""
import sys
import itertools
...
if __name__ == "__main__":
    ...
    # Distinct-untyped-support triangle screen. This omits repeated insertions and
    # branch/order data, so zero output does not prove Maurer--Cartan vanishing.
    mu2 = None
    if "--mu2" in sys.argv:
    ...
    print(f"\nFINITE-SCREEN SUMMARY: monogon table = {sum(mu0)} nonzero, "
          f"self-bigon table = {len(mu1)} nonzero"
          + (f", mu^2 = {mu2} nonzero" if mu2 is not None else " (mu^2: run --mu2)")
          + ". No full Maurer--Cartan conclusion is asserted.")
