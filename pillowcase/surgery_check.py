...
def torus_midpoint(first, second):
    """Midpoint along the short coordinatewise lift from first to second."""
    return tuple(
        (a + 0.5 * ((b - a + math.pi) % TAU - math.pi)) % TAU
...
def iota_pairing(E, c1, p1, c2, eps):
    ...
    for u, v in chords_for(ep1, p1):
        m = torus_midpoint(ep1[u][2], ep1[v][2])
    ...
    for p2 in (1, 2):
        ...
        for u, v in chords_for(ep2, p2):
            m = torus_midpoint(ep2[u][2], ep2[v][2])
