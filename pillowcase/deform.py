...
D_tab:
    ...
    D_tab(x) = sum_y [ #bigons(x,y) + sum_i c_i #tri(x,s_i,y) ] y   (mod 2).
...
def square_f2(M):
    """Return M^2 over F_2 for a square matrix M.

    This is deliberately independent of the polygon code so every finite matrix
    produced by an ancillary solver can be subjected to the same mandatory
    chain-complex sanity check.
    """
    n = len(M)
    if any(len(row) != n for row in M):
        raise ValueError("square_f2 requires a square matrix")
    return [[sum(M[i][k] & M[k][j] for k in range(n)) % 2
             for j in range(n)] for i in range(n)]


def square_entries_f2(M):
    """Return the nonzero entries of M^2 over F_2."""
    M2 = square_f2(M)
    return [(i, j) for i, row in enumerate(M2) for j, value in enumerate(row)
            if value]
...
if __name__ == "__main__":
    ...
    print(f"generators: {n}; D_big^2 audit: "
          f"{'PASS' if not raw_square else 'FAIL'}; nonzero entries {raw_square}")
    if raw_square:
        raise SystemExit("aborting before rank statistic: D_big^2 != 0")
    print(f"finite statistic h_big=n-2 rank(D_big)={n - 2 * rank_f2(d)} "
    ...
    print("\n=== finite-window triangle sweep over all 40 P self-crossings ===")
